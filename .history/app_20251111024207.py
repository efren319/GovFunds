...
def admin():
    ...
    try:
        ...
        """)
        unresolved_reports = reports_cur.fetchall()
    except Exception as e:
        # Fallback if there's an issue with the query
        projects = db.execute("SELECT * FROM projects ORDER BY id DESC LIMIT 50").fetchall()
        unresolved_reports = []
        print(f"Admin query error: {e}")
    
    return render_template('admin.html', projects=projects, unresolved_reports=unresolved_reports)
