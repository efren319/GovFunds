...
def budget():
    ...
    region_map = {
        ...
        "MIMAROPA": ["Mindoro Occidental","Mindoro Oriental","Marinduque","Romblon","Palawan"],
    ...
    region_labels = []
    region_data = [500000, 300000, 450000, 200000, 150000, 100000, 250000, 400000, 350000, 180000, 220000, 270000, 300000, 150000, 100000, 200000, 500000]

    for region, provinces in region_map.items():
        total = sum(
            p['allocated_budget'] for p in projects if p['department'] in provinces
        )
        region_labels.append(region)
        region_data.append(total)

    # DEBUG: check what you are passing
    print(region_labels)
    print(region_data)
