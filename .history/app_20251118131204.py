...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        import sys
        sys.stdout.flush()
        print(f"\n🔍 DECORATOR CHECK: Checking login for: {request.path}", file=sys.stderr)
        sys.stderr.flush()
        print(f"🔍 Session: {dict(session)}", file=sys.stderr)
        sys.stderr.flush()
        if 'admin_user' not in session or not session.get('admin_user'):
            print(f"❌ NOT LOGGED IN - REDIRECTING", file=sys.stderr)
            sys.stderr.flush()
            flash('Please log in to access the admin panel.', 'warning')
            return redirect(url_for('login'))
        print(f"✅ LOGGED IN AS: {session.get('admin_user')}", file=sys.stderr)
        sys.stderr.flush()
