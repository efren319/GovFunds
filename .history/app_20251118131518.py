...
def check_admin_access():
    ...
    # Skip check for resolve-report which has its own decorator
    if request.path == '/admin/resolve-report' or request.path.startswith('/admin/resolve-report/'):
        return
    
    # Check if current path requires login
    if request.path == '/admin':
        if 'admin_user' not in session or not session.get('admin_user'):
            print(f"🔐 ACCESS DENIED to {request.path} - Not logged in")
            flash('Please log in to access the admin panel.', 'warning')
            return redirect(url_for('login'))
        print(f"✅ ACCESS GRANTED to {request.path} - Logged in as {session.get('admin_user')}")
...
def test_session():
    ...
    return jsonify({
        'session_admin_user': session.get('admin_user', 'NOT SET'),
        'session_data': dict(session) if session else 'EMPTY',
        'session_id': request.cookies.get('session', 'NO COOKIE'),
        'all_cookies': dict(request.cookies)
    })

# Test admin route without decorator
@app.route('/test-admin-no-decorator')
def test_admin_no_decorator():
    return jsonify({
        'message': 'This route has NO decorator',
