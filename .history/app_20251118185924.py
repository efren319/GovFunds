...
def budget():
    try:
        ...
        # Get ALL available years first to see what data exists
        ...
        print(f"DEBUG: All available years in database: {years}")
        
        # If no years available, use the first one if it exists, otherwise use 2025
        if not years:
            print(f"DEBUG: No years found, using default 2025")
            years = [2025]
            selected_year = 2025
        elif selected_year not in years:
            # If selected year doesn't exist, use the first available year
            selected_year = years[0]
            print(f"DEBUG: Selected year {request.args.get('year', 2025)} not available, using {selected_year}")
        ...
        print(f"DEBUG: Department budgets for year {selected_year}: {len(department_budgets)} records")
        if department_budgets:
            print(f"DEBUG: First dept: {department_budgets[0].department} = {department_budgets[0].budget}")
        ...
        print(f"DEBUG: Regional budgets for year {selected_year}: {len(regional_budgets)} records")

        region_labels = [r.region for r in regional_budgets]
        region_data = [r.budget for r in regional_budgets]
        
        # Get annual budget for selected year from database
