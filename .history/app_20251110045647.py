...
def budget():
    ...
    for p in projects:
        ...
        department_totals[dept] += p['allocated_budget']

    dept_labels = list(department_totals.keys())
    dept_data = list(department_totals.values())
    ...
    for region, provinces in region_map.items():
        total = sum(
            p['allocated_budget'] for p in projects if p['region'] in provinces
        )
