document.addEventListener('DOMContentLoaded', () => {
    // Get department data from window object set by template
    const departmentsList = window.departmentsData?.list || [];

    // Transform data to match carousel format
    const transformedDepts = departmentsList.map(dept => ({
        name: dept.department,
        budget: dept.budget
    }));

    // Initialize carousel with department data
    if (typeof initDepartmentCarousel === 'function' && transformedDepts.length > 0) {
        initDepartmentCarousel(transformedDepts);
    }
